...
parser.add_argument('--val_dataset', default='./data/WIDER_FACE', help='Validation dataset directory')
...
parser.add_argument('--validation_gap', default=1, type=int, help='gap between interations for validation')
...
def train():
    ...
    dataset_val = VOCDetection(val_dataset, preproc(img_dim, rgb_mean), AnnotationTransform())
    ...
    for iteration in range(start_iter, max_iter):
        if iteration % args.validation_gap == 0:
            batch_iterator_val = iter(data.DataLoader(dataset_val, batch_size, shuffle=True, num_workers=num_workers, collate_fn=detection_collate))
            # load train data
            images_val, targets_val = next(batch_iterator_val)
            images_val = images_val.to(device)
            targets_val = [anno.to(device) for anno in targets_val]

            # forward
            out_val = net(images_val)
            loss_l_val, loss_c_val = criterion(out_val, priors, targets_val)
            loss_val = cfg['loc_weight'] * loss_l_val + loss_c_val
